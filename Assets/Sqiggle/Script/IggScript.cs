...
public class IggScript : PackUnpackableBehaviour,ParleyEnviromentInfo {
	...
	private int wildflowers=0;
	private bool showgold=false;
	private bool showwildflowers=false;
	...
	public void GetWildflowers(int howmuch){
		Parley.GetInstance().TriggerQuestEvent("GetMushroom");
	...
	public void ShowWildflowers(){
		showwildflowers=true;
	}
	
	public void SetWildflowers(float wildflowers){
		this.wildflowers=(int)wildflowers;
	}
	 ...
	 * returning the name, gold and wildflowers.
	 * 
	...
	public object GetEnviromentInfo(string key){
		...
		if (key.Equals("name")){
			...
			return gold;
		}else if (key.Equals("wildflowers")){
			return wildflowers;
		}
		return null;
	}
	...
	public void OnGUI(){
		...
		if (showwildflowers){
			label=label+" Wildflowers "+wildflowers+" " ;
		}
